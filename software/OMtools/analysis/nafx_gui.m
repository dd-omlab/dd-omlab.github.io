...
%              December 2001 - June 2019  (last mod: 06/30/19)
...
% June 2019: Updated to use 'select_emdata' function for data refresh/load/select
...
fig_height = 430;
tau = 'empty';
NL = newline;     % multi-platform carriage return
...
fErrFlag=0;
oldpath=pathsafe(pwd);
cd(pathsafe(findomprefs))
...
if ~ishandle(nafxFig)
   ...
   nafxFig = figure('Pos',[nafxXPos, nafxYPos, fig_width, fig_height],...
      ...
      'CloseRequestFcn',@(src,eventdata) nafx('done'),...
...
h.availDataH = uicontrol('Style','popup','Units','pixels',...
   ...
   'String',[{'Refresh Menu'};{'Get new data'}], ...
   'HorizontalAlignment','center','Value', 1, ...
   'Tooltip','Load or select data',...
   'Callback',@(src,eventdata) nafxAct('selectAvailData'));
...
h.plotactionH = uicontrol('Style','popup','Units','pixels',...
   ...
   'Callback',@(src,eventdata) nafxAct('plotaction'));
h.nafxprepH = uicontrol('Style','Push','Units','Pixels',...
   'Position', [x_orig+172 y_pos+1 60 25],'FontSize',fontsize,...
   'Tooltip',['Select segment start, stop, minfov and maxfov points ' NL,...
   'using "zoomtool''s" cursor1 button and "nafxprep" will ' NL,...
   'create a position-centered array  and a velocity array.'],...
   'String','nafxprep','Callback',@(src,eventdata) nafxprep);
...
h.nafx2snelH = uicontrol('Style','popup','Units','pixels',...
   ...
   'HorizontalAlignment','center',...
   'Tooltip',['Select the age of the subject (human), or "dog"' NL,...
...
h.posArrayNAFXH = uicontrol('Style','edit','Units','pixels',...
   ...
   'Tooltip',['Enter the name of the position segment to analyze.' NL,...
...
h.velArrayNAFXH = uicontrol('Style','edit','Units','pixels',...
   'BackgroundColor','cyan','ForeGroundColor','black',...
   'Tooltip',['Enter the name of the velocity segment to analyze.' NL,...
   'Automatically filled in when you use "nafxprep"'],...
   'FontSize',fontsize,...
...
h.posLimNAFXH = uicontrol('Style','popup','Units','pixels',...
   ...
   'Tooltip',['Enter the position window limit.' NL,...
   'It can be one of the following values:' NL,...
   '0.5, 0.75, 1.0, 1.25, 1.5. 2.0,' NL,...
...
h.velLimNAFXH = uicontrol('Style','popup','Units','pixels',...
   ...
   'Tooltip',['Enter the velocity window limit.' NL,...
   'It can be one of the following values:' NL,...
...
h.sampFreqH = uicontrol('Style','edit','Units','pixels',...
   ...
   'Tooltip',['Sampling frequency of the data. Should' NL,...
   'be automatically detected. It is used to' NL,...
...
h.fovCritNAFXH = uicontrol('Style','popup','Units','pixels',...
   ...
   'Tooltip',['Determines what criteria will be used to' NL,...
   'determine selection of raw foveation points.' NL,...
   'Default is "Pos & Vel"'],...
...
h.tauNAFXH = uicontrol('Style','edit','Units','pixels',...
   'BackgroundColor','cyan','ForeGroundColor','black',...
   'Position',[x_orig+100 y_pos 135 25],'FontSize',fontsize,...
   'Tooltip',['"Tau" is normally determined automatically' NL,...
   'based on the above position and velocity limits.' NL,...
   'If you manually enter a value here, it will' NL,...
...
h.dblPlotNAFXH = uicontrol('Style','popup','Units','pixels',...
   ...
   'Tooltip',['Determines how raw foveation points for the' NL,...
   'position and velocity segments are displayed.' NL,...
   'They can be plotted on the same figure, on' NL,...
...
h.fovStatNAFXH = uicontrol('Style','checkbox','Units','pixels',...
   ...
   'Tooltip',['Determines whether or not to output statistics' NL,...
...
h.fovCalcH = uicontrol('Style','Push','Units','Pixels',...
   ...
   'Tooltip',['Once you have entered all the above required' NL,...
   'values, click this button to see how many' NL,...
   'foveation periods meet the criteria. You can' NL,...
...
h.numFovNAFXH = uicontrol('Style','edit','Units','Pixels',...
   ...
   'Tooltip',['Displays the best guess of the number of foveations' NL,...
   'detected by the algorithm. This value can be replaced' NL,...
   'by your more accurate count of the actual number of ' NL,...
...
h.nafxCalcH = uicontrol('Style','Push','Units','Pixels',...
   ...
   'Tooltip',['After you have determined the number of foveations' NL,...
   '(and optionally modified the given value), click' NL,...
...
h.tauVersH = uicontrol('Style','checkbox','Units','Pixels',...
   ...
   'Tooltip',['Choose which version of the Tau Surface to' NL,...
   'use for NAFX calculations. The default is' NL,...
...
h.doneH = uicontrol('Style','Push','Units','Pixels',...
   ...
   'Tooltip',['When you''re sick and tired of calculating NAFXes' NL,...
   'you can make this go away. Go outside and enjoy' NL,...
