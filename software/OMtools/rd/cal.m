...
%
% 24 Aug 2022: 
%   Adds channel name to line's 'DisplayName' property in addition to 'Tag'
%   Added channel name to 'st' and 'sv' lines.
...
disp(['Zero offset: ' num2str(z_adjust,3) '   Time index: ' num2str(zeroPtTime)])
...
while i<=numcalpts+1
   ...
   if ~lSkipFlag
      ...
      disp(['Scaling factor for ' num2str(min_cal(i),3) deg ' (' dir2str ') is: ' ...
         num2str(min_scale(i)) '  Time index: ' num2str(min_cal_time(i-1),3)])
   else
      minScaleIndex(i)  = NaN;
...
if numcalpts==0 % zero offset only, use default values for r & l Str - added by PS
	...
	for i=2:numMaxCalpts
		...
		scaleStr = num2str(max_scale(i),3);
		disp([dir1str ' cal factor ' calPtStr deg ': ' scaleStr] )
	end
	rStr1 = mat2str(max_cal(2:numMaxCalpts),3);   % do not include the '0' first entry
	rStr2 = mat2str(max_scale(2:numMaxCalpts),3); % do not include the '0' first entry
	...
	for i=2:numLcalpts
		...
		scaleStr = num2str(min_scale(i),3);
		disp([dir2str ' cal factor ' calPtStr deg ': ' scaleStr] )
	end
	lStr1 = mat2str(min_cal(2:numLcalpts),3);
	lStr2 = mat2str(min_scale(2:numLcalpts),3);
	if numLcalpts == 2
		lStr1 = ['[' lStr1 ']'];
	else
		lStr2 = lStr2(2:end-1);
