...
% 18 Dec 23 -- Added better error checking detect presence of Eyelink Developer 
%              Kit software, needed for processing EDF files. Will warn if not 
%              present, and will direct user to check the platform-appropriate 
%              directory on their computer, and to download it from the 
%              SR-Support website.
% 20 Dec 23 -- No longer will attempt to copy 'edf2asc' from developer kit
%              directory to 'rd' directory. Not necessary now that there is
%              proper checking for 'edf2asc' presence and execution.
...
% devkitdir is the SR-created developer kit dir that contains the edf2asc binary
...
% bindir: directory that contains the edf2asc binary. Initially will be devkitdir

fsp = filesep;
%secret=0;
%bindir_err=0;
%bindir = 'no developer kit found';

% The Eyelink Developer kit should be installed in the directory for your OS:
if isunix
   if ismac
      edf2asc_name = 'edf2asc';  % removed the trailing spaces 04/13/20
      devkitdir='/Applications/Eyelink/EDF_Access_API/Example/';
      ...
      % Linux
      devkitdir='/usr/share/edfapi/EDF_Access_API/Example/';
      edf2asc_name = 'edf2asc';
   end
elseif ispc
   edf2asc_name = 'edf2asc.exe';
   devkitdir='C:\Program Files (x86)\SR Research\EyeLink\EDF_Access_API\Example\';
   %secret=0;
...
fprintf('Checking for ''edf2asc'' and the Eyelink developer kit...\n')
if ~exist(devkitdir,'dir')
   errorbeep
   fprintf('Error: I cannot find the Eyelink developer kit!\n')
   fprintf('It should be in the directory: ')
   if isunix
      if ismac
         fprintf('/Applications/Eyelink/EDF_Access_API/Example/')
         fprintf(' (MacOS)\n')
      else
         fprintf('/usr/share/edfapi/EDF_Access_API/Example/')
         fprintf(' (Linux)\n')
      end
   elseif ispc
      fprintf('C:\\Program Files (x86)')
      fprintf('\\SR Research\\EyeLink\\EDF_Access_API\\Example\\')
      fprintf(' (Windows)\n')
   else
      fprintf('Error: Unknown Operating System!!!\n')
   end
   fprintf('Please manually check the above directory for it.\n')
   fprintf('If it is not present you will need to download and install it.\n')
   fprintf('You can find it at ')
   fprintf('https://www.sr-support.com/forum/downloads/eyelink-display-software\n')
   fprintf('You will need a free SR-Support account to access it.\n')
   return
else
   % is edf2asc in the developer kit directory?
   
   fprintf('Found "edf2asc" in %s.\n',devkitdir)
end


% No longer copying 'edf2asc' to rd. Unnecessary now that there is better
% checking for developer kit directory.
% 'edf2asc' is NOT initially present in 'rd'.
%{
% If it exists in devkitdir, copy to 'rd'.
if secret<1
   % Is EL devkit installed?
   fprintf('I can try to copy "edf2asc" from the Developer kit dir to OMtools\n');
   try
      cd(devkitdir)
      fprintf('Eyelink developer kit dir exists: %s\n',devkitdir)
   catch
      %bindir_err=1;
      disp(['The directory ' devkitdir ' does not exist.'])
   ...
   if (exist(edf2asc_name,'file') ~= 2)
      %binfile_err=1;
      disp([edf2asc_name ' does not exist in' devkitdir])
      disp('Make sure that you have installed the Eyelink Developers Kit for')
      disp('for your platform. Login to the SR Support web site at:')
      disp('https://www.sr-support.com/forum/downloads/eyelink-display-software')
   else
   ...
   if strcmpi(yorn,'y')
      if isunix % mac or glx
         cmdstr=sprintf('cp %sedf2asc %s/edf2asc',devkitdir,rddir);
         try    system(cmdstr);
         catch, disp('Could not copy. Don''t know why.'); return; end
      else
         %is win
         cmdstr=sprintf('cp %sedf2asc.exe %s/edf2asc.exe',devkitdir,rddir);
         try    system(cmdstr);
         catch, disp('Could not copy. Don''t know why.'); return; end
      end
   else
...
if exist([rdp fsp edf2asc_name],'file')
   ...
   % Use default version.
   binfile=[devkitdir edf2asc_name];
end
...
fprintf('Events and messages have been exported.\n');
...
%a=[binfile ' ' inputfile ' ' datafile exp ' -s -t -y -nflags -miss NaN']; %%TEST
system(a);
...
% Did edf2asc actually run? 
if ~contains(sysresult,'EyeLink EDF file')
   errorbeep
   fprintf('ERROR!!! EDF2ASC executable did not run!!!\n')
   fprintf('Make sure that the Eyelink Developer kit has been installed.\n')
   fprintf('It should be in the directory: ')
   if isunix
      if ismac
         fprintf('/Applications/Eyelink/EDF_Access_API/Example/')
...
% edf2asc ran, but could not process the EDF file.
if contains(sysresult,'Could not process')
   ...
   success = 1;
   fprintf('I have processed the EDF file.\n')
   delete('edf_error.txt')
end
