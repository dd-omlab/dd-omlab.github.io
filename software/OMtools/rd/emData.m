classdef emData < dynamicprops
   ...
   %
   % To COPY an emData object to a new variable, use 'emcopy'.
   %    e.g. 'emd1 = emcopy(emd);'
   % Simply trying 'emd1 = emd;' will just create a REFERENCE to the original
   % object, so any changes made to emd1 would actually modify emd.
   ...
   properties
      ...
      stop_times  = []; % if sharing common timebase w/external data samples
      filename = '';
      pathname = '';
      comments = '';
      chan_names = {};
      ...
      camdist=struct('dist',[],'channel','dd');
      ...
      rp=struct('size',[],'channel','rp','chan_comment',[],...
         'unmod',[]);
      lp=struct('size',[],'channel','lp','chan_comment',[],...
         'unmod',[]);
      dd=struct('dist',[],'channel','dd','chan_comment',[],...
         'unmod',[]);
